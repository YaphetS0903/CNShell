...
- 多人终端新增 relay 无关的本机协议状态机与加密传输层：活动成员/设备校验、默认只读、10–300 秒单持有者控制租约；指定设备邀请通过 X25519/HKDF/AES-GCM 封装房间密钥，输入/输出帧由发送设备签名并端到端加密，支持 5 分钟/512 帧/4 MB 的内存断线补帧以及角色、设备或 epoch 变化后的立即失效。正式网络 relay 尚未启用。
...
- 团队设置已接入 relay 账号注册/登录、Keychain 短期会话、Ed25519 设备自动刷新、工作区发布、在线邀请接受、成员/设备/epoch 同步、服务端角色变更和设备撤销；多人终端房间 WebSocket 与观看/控制入口仍待后续接通。
...
- 团队终端加密协议、relay 服务端和客户端账号/工作区同步已完成，但房间 WebSocket/观看控制入口、正式域名/TLS/WSS/邮件/监控/备份和跨设备真机会话仍未完成；X/Ymodem/Kermit 的实体串口及第三方设备互操作待补。
