...
- 修复 macOS 与 Windows 桌面 WebView 未可靠显示系统口令提示时，“导出含凭据的加密备份”仍继续提交空口令并报错的问题。
- 加密导出改为先在应用内设置至少 8 位口令并二次确认，校验通过后再选择保存位置；提供口令显隐控制、就地错误提示和处理中状态。
- 加密导入在选中备份后直接显示应用内口令表单，保留已选择的文件供错误口令重试，不再依赖系统 `prompt`。

### 安全与兼容性

- 含凭据备份继续使用 Argon2id 派生密钥与 AES-256-GCM 加密；口令不会保存或上传，错误口令不会写入部分连接。
- 备份格式保持不变，可在 macOS 导出后导入 Windows；密码与私钥口令在目标电脑写入其系统凭据库。
- 新增加密导出前置校验、无系统提示调用、Windows 路径导入和单次文件选择回归测试；既有加密备份往返与错误口令拒绝测试继续通过。

## 0.2.0-beta.12（工作区体验与自动化中心版）

### 新增

- 自动化从设置页拆分为独立中心，提供任务列表、步骤编辑、自然语言计划、时区、下次运行、持久运行记录、手动运行、Python 和录制四个子页。
- 终端新增 AI 辅助入口，支持携带选区或错误上下文、发送前脱敏预览和明确确认；AI 设置仅保留 Provider 管理。
- 主工作区新增终端优先、文件优先和监控优先布局，以及单面板放大；基础设置新增 90%、100%、110% 和 125% 全局界面缩放与即时预览。
- 文件管理器新增单行可编辑路径栏，将面包屑与完整路径输入合并，并区分根目录和真实远端用户主目录入口。

### 优化

- 连接编辑器按协议保存独立草稿，切换 SSH、RDP、Serial、本地 Shell 和 Telnet 时不再带入无关字段；精简首屏并增加固定操作区与“保存并连接”。
- 设置中心明确基础设置与各模块的独立保存范围，统一输入框、下拉框、复选框和表单间距，并为插件、MCP、团队、备份、同步和代理提供更清晰的首次配置引导。
- 传输队列增加状态计数、连接筛选、失败突出和可操作的中文错误说明；连接诊断、隧道、批量执行、快捷命令、通知规则、粘贴历史、帮助、关于和更新入口同步整理。
- 网络监控固定主接口并按会话和网卡保存历史；系统信息、进程、端口和连接列表补充采集时间、筛选、分页与高负载保护。
- 标题栏显示当前连接名称、协议和身份；移除重复的连接管理器和新建连接按钮，保留连接库切换与左侧新建入口。

### 无障碍与可靠性

- 深色与浅色主题的辅助文字在最不利语义背景上达到 WCAG AA 4.5:1，并加入源码变量和 WebKit 最终计算样式回归测试。
- 改进弹窗初始焦点、焦点返回、Escape/点击外部关闭及紧凑窗口布局；125% 缩放下主要弹窗仍保留固定操作区。
- 自动化运行结果写入 SQLite；端口与连接列表按 200 条分批渲染，避免一次创建最多 5000 行界面。

### 验证

- 前端单元测试 315/315、WebKit 端到端测试 24/24、Rust 测试 278/278、发布与 updater 定向测试 69/69 通过；ESLint、TypeScript、生产构建和 `git diff --check` 通过。
- AI、插件、MCP、团队、Relay、RDP 与 Serial 本地定向测试 71/71 通过；本地 SSH/SFTP 互操作 14/14 通过。
- 60 秒 SSH 稳定性测试完成 30 次操作，PTY 往返 0 ms，RSS 从 59,792 KiB 降至 59,760 KiB，峰值 59,824 KiB。
- GitHub Beta.11 在线清单与仓库 SHA-256 一致，macOS updater 归档已由 CNshell 内置 verifier 确认签名与 release 公钥匹配。

### 已知限制

- macOS 继续使用 ad-hoc 签名且没有 Developer ID/公证；Windows 没有 Authenticode。GitHub Releases 与 Tauri minisign 一键更新不受影响，但首次安装仍可能显示 Gatekeeper 或 SmartScreen 提示。
- Windows ARM64、Intel Mac、真实 RDP、实体串口、Mosh 漫游、WebDAV 双设备、生产 Relay 与完整辅助功能矩阵仍需对应设备或服务继续验收。

## 0.2.0-beta.11（传输、编辑器与系统信息可靠性版）

### 新增

- 系统信息重构为“概览、CPU 与内存、网络、磁盘、端口与连接”五个固定分类；新增内核名称、运行时间、负载、CPU 分项占用、内存/Swap 明细、网络累计流量和实时速率，并兼容 ARM Linux 的 `lscpu` 硬件信息。
- 远程文本编辑器提升为应用级宿主，支持按连接和路径恢复本机草稿、三方冲突处理，以及切换会话、关闭会话和退出应用时的未保存内容保护。
- 底部工具面板新增常驻按钮、终端右键菜单入口和 `⌘J`/`Ctrl+J` 快捷键；帮助页补充明确操作说明，重新打开时保留上次工具页。
- 新增真实 OpenSSH/SFTP 传输验收入口，覆盖四路并发、暂停/恢复/取消、冲突策略、失败重试和最终文件完整性。

### 修复

- 传输事件改由应用级监听，即使关闭或切换传输面板，进度、速度和最终状态仍持续同步；退出时会提示未完成任务。
- 修复下载冲突改名后数据库仍记录旧目标、SCP 长度校验误判、同名连接双击错配和并发传输目标冲突等问题。
- 修复编辑器切换工具页或会话后丢失内容、关闭期间保存竞态、外部编辑副本未回传以及草稿空间不足仍允许关闭的问题。
- 修复应用退出只销毁窗口却遗留后台进程的问题，统一由前端检查未保存编辑和后台传输后调用后端退出。
- 全局隐藏视觉滚动条，同时保留鼠标滚轮、触控板和键盘滚动能力，减少界面中的高亮滚动轨道。

### 验证

- 前端 64 个测试文件、272 项测试通过；Rust 监控模块 10 项测试通过，生产构建、ad-hoc 签名校验及 macOS ARM 云服务器实时系统信息验证通过。
- 完整传输验收覆盖 1 GiB 往返、10 万目录项、真实队列与 SHA-256 一致性；本轮发布不重复执行长时间 soak。

### 已知限制

- macOS 仍未配置 Developer ID 与公证；Windows 仍未配置 Authenticode。x64 保持 Beta，ARM64 保持 Preview。
- Windows ARM64、Intel Mac、不同 macOS 版本与完整弱网/辅助功能矩阵仍需对应外部环境继续验收。

## 0.2.0-beta.10（大屏面板字号适配版）

### 新增

- 监控栏和远端文件管理区新增独立字号调节控件（10–16px）；自动模式会结合显示器分辨率与系统 DPI，在低缩放 2K/4K 大屏上分别提高到 12/13px，避免与 Windows 系统缩放重复放大。
- 目录树、文件表格、磁盘详情和操作提示会同步缩放；手动字号按面板分别保存在本机，重启 CNshell 或切换连接后仍保留，点击“字”图标可恢复自动模式。

## 0.2.0-beta.9（Windows SSH 私钥兼容性修复版）

### 新增

- Windows 构建现在默认使用随应用打包的 OpenSSL SSH 后端，支持现代 OpenSSH 私钥格式与加密私钥。
...
- 更新 Browserslist 及相关锁文件依赖，修复新披露的高危内存增长与原型写入漏洞，确保 npm audit 安全门禁通过。
...
- 设置中心重组为基础设置、连接与安全、自动化与集成、团队与云端、关于与支持五个分类，降低普通用户查找常用选项的成本。
- 新增设置搜索与结果跳转、上下文帮助、基础默认值恢复、固定取消/保存栏和未保存更改保护。
- 高级设置模块支持折叠与按需加载；切换分类、展开模块和搜索跳转时保留尚未保存的草稿。

### 修复

- 修复 WebKit/VoiceOver 中切换设置分类后，重新激活的模块可能从辅助功能树消失的问题。
- 完善设置窗口的键盘焦点回绕、Escape 关闭、语义化分类、模块按钮和搜索结果焦点行为。

### 验收

- Windows 11 Build 26200、200% DPI 与 Narrator 已完成设置中心五分类、固定操作栏及搜索 `Mosh` 跳转验收。
- macOS VoiceOver 已完成五分类、全部设置模块、搜索跳转、上下文帮助、焦点回绕和 Escape 关闭验收。

### 已知限制

- macOS 仍未配置 Developer ID 与公证；Windows 仍未配置 Authenticode。x64 保持 Beta，ARM64 保持 Preview。
- 当前读屏结论仅覆盖设置中心，不扩大为整个应用；Windows ARM64、Intel Mac、不同 macOS 版本、真实 RDP 与完整弱网等外部矩阵仍需对应环境继续验收。
...
- 新增 SFTP 通道超时恢复、跨连接上传锁、目录树懒恢复、严格权限解析和窗口销毁失败恢复的自动化回归测试。

### 修复

- 修正 SSH Jump 与端口转发中 libssh2 超时单位错误，避免原本应为 20 秒的网络操作被误设为 20 毫秒。
- 为 SFTP 辅助连接排队、建连和普通网络调用增加有界超时；超时后会丢弃失效 transport 并重建通道，避免目录持续转圈或只能重启应用恢复。
- 文件树恢复时只自动加载根目录和当前活动路径，历史展开目录保持懒加载；切换快捷命令、终端或文件视图后不再回到初始位置，也不会产生批量后台请求。
- 上传目标锁加入连接 ID，不同服务器上的同名远端路径可并行传输；下载仍按本地目标防止冲突覆盖。
- 窗口销毁失败时恢复关闭按钮状态并显示错误，不再让红色关闭按钮永久失效。
- 文件权限只接受完整三位或四位八进制值，拒绝部分匹配及其他非法输入。

### 优化

- CodeMirror 语法包按远端文件类型动态加载并提供纯文本回退，降低编辑器首包体积与打开文件时的阻塞。
- 智能命令排序只归一化一次查询文本，减少候选列表计算开销。
...
- 新增 Windows Beta.4 桌面启动与主题回归验证项，覆盖 GUI 子系统、原生标题栏、菜单行为和终端配色。
- 新增默认关闭的本机 MCP Server：支持 13 个受控 SSH/SFTP 工具、客户端/连接/目录授权、短期会话、逐次审批、脱敏审计和原生选择器本地文件 grant；随 macOS universal 与 Windows x64/ARM64 安装包分发 `cnshell-mcp`。

### 修复

- MCP 精确命令规则现在只接受保守低风险命令；中高风险命令不能通过界面或 Broker 保存为后续自动批准。
- 升级到此版本会撤销早期 MCP 预览版的精确命令规则，避免旧的宽松策略继续生效；符合当前低风险规则的命令可在下次审批时重新保存。
...
- Windows 安装器门禁改用显式标准输出重定向验证 GUI 子系统程序，避免把没有控制台附着误判为启动失败。
