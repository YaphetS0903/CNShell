...
describe("TextEditor",()=>{
  ...
  it("selects a Windows executable instead of a macOS application directory",()=>{expect(externalApplicationDialogOptions({operatingSystem:"windows",displayName:"Windows"},"选择Windows外部文本编辑器")).toEqual({multiple:false,directory:false,defaultPath:undefined,title:"选择Windows外部文本编辑器",filters:[{name:"Windows 应用",extensions:["exe","com"]}]});expect(externalApplicationDialogOptions({operatingSystem:"macos",displayName:"macOS"},"选择macOS外部文本编辑器")).toEqual({multiple:false,directory:true,defaultPath:"/Applications",title:"选择macOS外部文本编辑器"});});
});
