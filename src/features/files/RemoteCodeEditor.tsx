...
export const RemoteCodeEditor=forwardRef<CodeEditorActions,{path:string;value:string;onChange:(value:string)=>void}>(({path,value,onChange},ref)=>{
  ...
  useEffect(()=>{const host=hostRef.current;if(!host)return;const view=new EditorView({parent:host,state:EditorState.create({doc:initialValueRef.current,extensions:[basicSetup,languageForPath(path),EditorView.lineWrapping,EditorView.contentAttributes.of({"aria-label":"远程文本内容","aria-multiline":"true"}),EditorView.updateListener.of((update)=>{if(update.docChanged)onChangeRef.current(update.state.doc.toString());}),EditorView.theme({"&":{height:"100%",backgroundColor:"#07101d",color:"#dce6f4"},".cm-scroller":{fontFamily:"SFMono-Regular, Menlo, Monaco, monospace",fontSize:"12px",lineHeight:"1.55",overflow:"auto"},".cm-gutters":{backgroundColor:"#0b1524",color:"#64748b",borderRight:"1px solid #23324a"},".cm-activeLine,.cm-activeLineGutter":{backgroundColor:"#172337"},".cm-selectionBackground,.cm-content ::selection":{backgroundColor:"#315b8e88!important"},".cm-cursor":{borderLeftColor:"#4ade80"},".cm-foldPlaceholder":{backgroundColor:"#172337",border:"1px solid #334155",color:"#94a3b8"}})]})});viewRef.current=view;return()=>{view.destroy();viewRef.current=null;};},[path]);
