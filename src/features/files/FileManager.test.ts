...
describe("remote file virtualization",()=>{
  ...
  it("accepts only complete octal permission values",()=>{expect(parseRemoteMode("755")).toBe(0o755);expect(parseRemoteMode(" 0644 ")).toBe(0o644);expect(parseRemoteMode("755junk")).toBeNull();expect(parseRemoteMode("888")).toBeNull();expect(parseRemoteMode("")).toBeNull();});
});
