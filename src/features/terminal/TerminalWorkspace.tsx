...
export default function TerminalWorkspace({connect}:{connect:(profile:ConnectionProfile)=>Promise<void>}) {
  ...
  const close = useCallback(async (id: string) => { const session=useAppStore.getState().sessions.find((item)=>item.id===id);if(id===splitSessionId||Boolean(splitSessionId&&id===useAppStore.getState().activeSessionId))setSplitSessionId(null);try { if(session?.sessionType==="rdp")await api.rdpClose(id);else await api.closeTerminal(id); } catch { /* already disconnected */ } removeSession(id); refs.delete(id); },[refs,removeSession,splitSessionId]);
  ...
  const selectSession=useCallback((id:string)=>{if(id===splitSessionId)setSplitSessionId(null);setActiveSession(id);},[setActiveSession,splitSessionId]);
  const split=async(session:TerminalSession)=>{if(session.sessionType==="rdp")return;if(splitSessionId){setSplitSessionId(null);return;}const profile=profileFor(session);if(!profile)return;const before=new Set(useAppStore.getState().sessions.map((item)=>item.id));await connect(profile);const created=useAppStore.getState().sessions.find((item)=>!before.has(item.id));if(created){setSplitSessionId(created.id);setActiveSession(session.id);}};
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (event.metaKey && /^[1-9]$/.test(event.key)) { const session = sessions[Number(event.key)-1]; if (session) { event.preventDefault(); selectSession(session.id); } }
  ...
  return <main className="workspace">
    <div className="session-tabs" role="tablist" aria-label="打开的会话" onKeyDown={(event)=>moveTabFocus(event,sessions.map((session)=>session.id),active.id,selectSession,"session-tab-")}>
      {sessions.map((session) => <div className="session-tab-wrap" key={session.id}><button id={`session-tab-${session.id}`} role="tab" aria-selected={session.id === active.id} aria-controls={`session-panel-${session.id}`} tabIndex={session.id===active.id?0:-1} aria-label={`${session.title}，${sessionStatusLabel(session.status)}${session.lastError?`，${session.lastError}`:""}`} className={`session-tab ${session.id === active.id ? "active" : ""}`} onClick={() => selectSession(session.id)}><span className={`status-dot ${session.status}`} aria-hidden="true"/><span>{session.title}</span>{session.status!=="online"&&<small>{sessionStatusLabel(session.status)}</small>}</button><IconButton icon={MoreHorizontal} label={`${session.title} 会话操作`} className="tab-menu-trigger" aria-haspopup="menu" aria-expanded={tabMenu===session.id} onClick={()=>setTabMenu(tabMenu===session.id?null:session.id)}/>{tabMenu===session.id&&<div className="tab-context-menu" role="menu"><button role="menuitem" onClick={()=>void duplicate(session)}><Copy size={13}/>复制会话</button><button role="menuitem" onClick={()=>void reconnect(session)}><RefreshCw size={13}/>重新连接</button>{session.sessionType==="terminal"&&<button role="menuitem" onClick={()=>void split(session)}><Columns2 size={13}/>{splitSessionId?"关闭拆分":"左右拆分"}</button>}<button role="menuitem" className="danger" onClick={()=>{if(!settings.confirmCloseActiveSession||confirm(`关闭“${session.title}”会话？`))void close(session.id);}}><X size={13}/>关闭</button></div>}</div>)}
