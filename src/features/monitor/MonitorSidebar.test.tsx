...
describe("monitor history",()=>{
  ...
  it("lets users enlarge the monitor text and remembers the choice",async()=>{localStorage.clear();const user=userEvent.setup();useAppStore.setState({sessions:[{id:"session",connectionId:"connection",sessionType:"terminal",title:"server",status:"online",startedAt:"",lastError:null}],activeSessionId:"session",monitor:{sessionId:"session",timestamp:0,hostname:"server",ip:"127.0.0.1",uptimeSeconds:0,load:[0,0,0],cpuPercent:0,memoryUsedBytes:0,memoryTotalBytes:0,swapUsedBytes:0,swapTotalBytes:0,latencyMs:null,processes:[],disks:[],networks:[],warnings:[]}});const first=render(<MonitorSidebar/>);expect(screen.getByTitle("监控当前字号")).toHaveTextContent("自11px");await user.click(screen.getByRole("button",{name:"增大监控字号"}));expect(screen.getByTitle("监控当前字号")).toHaveTextContent("12px");expect(localStorage.getItem("cnshell-monitor-font-size")).toBe("12");first.unmount();render(<MonitorSidebar/>);expect(screen.getByTitle("监控当前字号")).toHaveTextContent("12px");});
