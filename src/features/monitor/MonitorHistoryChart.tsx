...
type Metric = "network" | "latency";

export function MonitorHistoryChart({
  samples,
  metric,
}: {
  samples: MonitorHistorySample[];
  metric: Metric;
}) {
  const host = useRef<HTMLDivElement>(null);
  const chart = useRef<uPlot | null>(null);
  const ready = samples.length >= 2;
  const data = useMemo(() => chartData(samples, metric), [metric, samples]);
  ...
  useEffect(() => {
    ...
    const instance = new uPlot(
      {
        width: Math.max(120, element.clientWidth),
        height: 54,
        cursor: { show: false },
        legend: { show: false },
        ...
        scales: {
          x: { time: true },
          y: {
            range: (_u, min, max) => [0, Math.max(1, max ?? min ?? 1)],
          },
        },
        series: network
          ? [
              ...
              {
                label: "下载",
                stroke: style.getPropertyValue("--accent").trim(),
                width: 1.4,
              },
              {
                label: "上传",
                stroke: style.getPropertyValue("--blue").trim(),
                width: 1.4,
              },
            ]
          : [
              ...
              {
                label: "延迟",
                stroke: style.getPropertyValue("--warning").trim(),
                width: 1.4,
              },
            ],
      },
  ...
  return (
    <div
      className="monitor-history-chart"
      ref={host}
      role="img"
      aria-label={label}
    />
...
function chartData(
  ...
  return metric === "network"
    ? [
        timestamps,
        samples.map((sample) => sample.received),
        samples.map((sample) => sample.sent),
      ]
