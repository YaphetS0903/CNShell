...
import type { ConnectionProfile, TeamAuditEvent, TeamDevice, TeamMember, TeamPermissionReport, TeamSharePreview, TeamWorkspace } from "../../types";
...
export function TeamSettings({
  ...
  const saveMember = async (member?: TeamMember, role = memberRole) => {
    ...
    try { setBusy(true); await api.saveTeamMember({ workspaceId, memberId: member?.id ?? null, displayName, role }); if (!member) setMemberName(""); await Promise.all([refreshWorkspace(workspaceId), refreshWorkspaces()]); }
    catch (error) { onError(errorMessage(error)); } finally { setBusy(false); }
  };

  const removeMember = async (member: TeamMember) => {
    if (!confirm(`移除成员 ${member.displayName}？后续共享会切换到新的密钥 epoch。`)) return;
    try { await api.removeTeamMember(workspaceId, member.id); await Promise.all([refreshWorkspace(workspaceId), refreshWorkspaces()]); }
    catch (error) { onError(errorMessage(error)); }
  };
  ...
  const revokeDevice = async (device: TeamDevice) => {
    ...
    try { await api.revokeTeamDevice(workspaceId, device.id); await Promise.all([refreshWorkspace(workspaceId), refreshWorkspaces()]); }
    catch (error) { onError(errorMessage(error)); }
  };
  ...
  return <section className="plugin-settings" aria-label="团队工作区">
    <div className="section-heading"><div><h3><Users size={16}/> 团队工作区与 RBAC</h3><p>本机权限、设备密钥和加密分享已启用；在线邀请、同步与多人会话仍需正式团队服务。</p></div></div>
    ...
    {workspaces.length > 0 && <>
      ...
      <div className="plugin-report"><strong><KeyRound size={14}/> 设备密钥</strong>{!localDevice && <div className="backup-actions"><input aria-label="本机设备名称" placeholder="例如：Chen 的 Mac" value={deviceName} onChange={(event) => setDeviceName(event.target.value)} maxLength={256}/><button className="mini-button" disabled={busy || !deviceName.trim()} onClick={() => void ensureDevice()}><KeyRound size={13}/>创建设备身份</button></div>}{localDevice && <div className="backup-actions"><button className="mini-button" onClick={() => void exportDevice()}><Download size={13}/>导出本机公钥</button>{canManageShares && <button className="mini-button" onClick={() => void importDevice()}><Upload size={13}/>导入设备公钥</button>}</div>}{devices.map((device) => <div className="plugin-record" key={device.id}><div><strong>{device.name}</strong><small>{members.find((member) => member.id === device.memberId)?.displayName ?? device.memberId} · {device.isLocal ? "本机" : "远端"} · {device.status === "active" ? "活动" : "已撤销"}</small><small>SHA-256 {device.fingerprint.replace("sha256:", "")}</small></div>{device.status === "active" && canManageShares && <button className="icon-button" aria-label={`撤销设备 ${device.name}`} onClick={() => void revokeDevice(device)}><Trash2 size={14}/></button>}</div>)}</div>
