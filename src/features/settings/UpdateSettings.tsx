...
const candidateChannelMessage =
  "当前开发构建未配置更新通道。GitHub 发布包支持签名的一键更新。";
...
export function UpdateSettings({
  ...
  const checkNow = async () => {
    ...
    try {
      ...
      if (!update) {
        setState({ status: "current", message: "当前已是最新版本。" });
        return;
      }
      ...
      setState({
        status: "available",
        update,
        message: `发现 CNshell ${update.version}（当前 ${update.currentVersion}）`,
      });
    } catch (reason) {
  ...
  const install = async (update: Update) => {
    if (
      !confirm(
        `下载并安装 CNshell ${update.version}？安装完成后请重新启动应用。`,
      )
    )
      return;
    let received = 0,
    ...
    try {
      ...
      setState({
        status: "current",
        message: "更新已安装。请退出并重新启动 CNshell。",
      });
    } catch (reason) {
    ...
    <section>
      ...
      <div className="backup-actions">
        ...
        >
          <RefreshCw
            size={14}
            className={state.status === "checking" ? "spin" : undefined}
          />
          检查更新
        </button>
