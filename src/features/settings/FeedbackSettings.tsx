...
export function FeedbackSettings({onError}:{onError:(message:string)=>void}) {
  ...
  const environmentLabel=useMemo(()=>environment?`CNshell ${environment.appVersion} · ${platform.displayName} ${environment.osVersion} · ${environment.architecture}`:status,[environment,platform.displayName,status]);
