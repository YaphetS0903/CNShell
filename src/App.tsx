...
export default function App() {
  ...
  useEffect(()=>{if(!api.isDesktop())return;let closing=false;const listener=getCurrentWindow().onCloseRequested(async(event)=>{event.preventDefault();if(closing)return;const activeTransfers=useAppStore.getState().transfers.filter((task)=>["queued","running","paused"].includes(task.status));if(activeTransfers.length&&!confirm(`仍有 ${activeTransfers.length} 个传输任务未完成。关闭 CNshell 会中断这些任务，确定继续吗？`))return;closing=true;await saveBeforeWindowClose(()=>workspacePersistenceReady.current?api.saveWorkspace(workspaceSnapshotRef.current()):Promise.resolve(),()=>getCurrentWindow().destroy(),(reason)=>console.error("工作区状态保存失败",reason));});return()=>{void listener.then((unlisten)=>unlisten());};},[]);
