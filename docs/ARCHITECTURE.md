...
- `src-tauri/src/mcp.rs`：MCP Broker、客户端授权、审批、审计、短期会话和 SSH/SFTP 工具适配。
- `src-tauri/src/mcp_protocol.rs` 与 `src-tauri/src/bin/cnshell-mcp.rs`：有界本机 Broker 协议和基于 `rmcp` 的 stdio sidecar。
...
外部 MCP Host 只启动随应用分发的 stdio sidecar。sidecar 使用固定版本 `rmcp` 解析 JSON-RPC，再通过带长度前缀的本机 loopback 请求调用 CNshell Broker；标准输出只承载 MCP，诊断写标准错误。Broker 复用现有 SSH `SessionManager`、Transport Pool 和 SFTP 原子操作，不复制密码或建立独立凭据体系。

Broker generation 与 token 每次应用启动轮换，客户端 secret 与本地路径 grant 放在系统凭据库。SQLite 只保存授权关系、路径 basename 和脱敏审计；审批与短期 session 只在内存中。写操作必须由后端审批通道返回一次性决定，WebView 只展示和提交 approve/reject。上传和下载由后端直接访问原生选择器授权的路径，文件正文不经过 MCP JSON 或 WebView IPC。
