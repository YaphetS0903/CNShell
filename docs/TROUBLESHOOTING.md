...
- **要确认 sidecar 是否完整安装**：在 MCP 设置为已登记客户端生成配置后，复制“安装后自检命令”并在本机终端执行。预期只输出 JSON，包含 `status: "ok"`、版本、`transport: "stdio"`、13 个工具、4 个 Resources 和 2 个 Prompts；它不需要或接触远端 SSH 凭据。
...
- **系统信息字段被拒绝**：`cnshell_system_info.fields` 只能使用 `os`、`cpu`、`memory`、`load`、`disks`、`network`，不得重复；留空可取完整固定快照。该工具不接受 shell、路径或自定义探测脚本。
