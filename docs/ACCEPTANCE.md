...
| AI、插件、团队协作 | 部分（客户端在线接入/外部服务验收待补） | AI、可信插件沙箱、本地团队 RBAC/设备/审计、Keychain 设备密钥 E2E 离线连接分享和团队终端加密协议已完成。独立 relay 服务端已实现 Argon2id 账号、短期 token、设备 challenge、工作区邀请、服务端 RBAC/epoch/撤销、密文 WebSocket、有界补帧、审计和永久删除；双账号双设备 loopback 覆盖断线、控制和撤销。客户端在线账号/工作区同步/多人终端 UI、正式域名/TLS/WSS/邮件/监控/备份和真实双设备网络会话仍未完成，不声明正式在线功能可用 |
