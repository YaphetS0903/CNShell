...
| AI、插件、团队协作 | 部分（多人终端在线接入/外部服务验收待补） | AI、可信插件沙箱、本地团队 RBAC/设备/审计、Keychain 设备密钥 E2E 离线连接分享和团队终端加密协议已完成。relay 服务端及客户端 HTTPS 账号、Keychain token、设备 challenge、工作区发布/邀请/目录同步已完成双账号 loopback。客户端房间 WebSocket/观看控制 UI、正式域名/TLS/WSS/邮件/监控/备份和真实双设备网络会话仍未完成，不声明正式在线终端可用 |
...
| 保留验收边界 | 加密客户端协议已完成，但客户端在线 UI 和正式 relay 部署仍未完成，也没有两台真实设备的跨网络会话；因此在线多人终端入口继续关闭，本机回环不冒充真实网络或生产服务验收 |
...
| 自动化证据 | 真实 loopback HTTP/WebSocket 集成测试启动两个账号与两台设备，覆盖注册、邀请、RBAC 拒绝、challenge、签名密文、断线补帧、控制输入定向、重复拒绝、成员移除、epoch、token 失效、审计隐私和永久删除；relay 独立 `cargo clippy --all-targets -- -D warnings` 与 `cargo test` 通过 |
...
| 保留验收边界 | 尚未接通房间 WebSocket 和观看/控制 UI，也没有正式 TLS 域名、邮件、监控备份或两台真实设备跨网络证据；本项不等于在线多人终端验收通过 |
| 部署边界 | 提供非 root、只读文件系统兼容的 Dockerfile 和仅绑定 host loopback 的 Compose 示例；非 loopback 启动必须显式确认位于 TLS 代理后。当前机器没有 Docker，未执行镜像构建；正式 DNS/TLS/WSS、邮件、代理限速、加密卷、备份恢复、监控告警和真实两设备仍待外部环境 |
