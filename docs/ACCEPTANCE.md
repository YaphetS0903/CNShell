...
| 发布 workflow 供应链门禁 | 本机通过，远端升级待复验；CI/release 所有外部 `uses:` 均固定为 40 位 commit SHA，Dependabot 已把 `checkout/setup-node/upload-artifact` 升级至 7.0.0/7.0.0/7.0.1，Node `20.20.2`、Rust `1.96.0` 固定；执行 Clippy 的 workflow 显式安装 minimal profile 不包含的组件。15 个 release-script 定向测试验证精确引用、Clippy 组件、拒绝浮动 tag，并保证 `.p12`、`.p8`、私有 release 配置和临时 Keychain 在 Artifact Action 前清理，只有构建与清理均成功才上传。GitHub Actions run `29525490667`（CI #75）证明第一版固定 Action、Node 与 Rust 均成功解析，但 `Frontend and Rust` 因未安装 `cargo-clippy` 失败；当前 Clippy 修复与 Actions 升级须由后续 CI 复验，不把 #75 记为通过 |
