...
| AI、插件、团队协作 | 部分（本机代码完成/在线服务待建） | AI、可信插件沙箱、本地团队 RBAC/设备/审计和 Keychain 设备密钥 E2E 离线连接分享已完成。插件提供逐项显式授权的 `ui`、精确域名 HTTPS、用户 Bookmark 目录、终端选区/待确认输入、脱敏连接元数据和一次性凭据代理 ABI；未知导入、未授权能力、私网解析、重定向、符号链接与越界均阻断。多人终端已完成主持状态机、指定设备邀请密钥封装、发送设备签名的 AES-GCM 帧、严格输入/输出序号、短控制租约，以及 5 分钟/512 帧/4 MB 内存重放窗口；两份独立客户端数据库和各自 Keychain 设备密钥的本机回环覆盖补帧、篡改、重放与撤销。在线账号/设备目录、服务端 RBAC/撤销传播、生产 WSS relay、跨设备 UI 和真实双设备网络会话仍需正式服务与设备，不声明完成 |
...
| 保留验收边界 | 尚未部署生产 WSS relay、在线账号/邀请/设备目录和服务端 RBAC/撤销传播，也没有两台真实设备的跨网络会话；因此在线多人终端 UI 继续关闭，本机回环不冒充真实网络或服务端验收 |
