...
| AI、插件、团队协作 | 部分 | AI、可信插件沙箱、本地团队 RBAC/设备/审计和 Keychain 设备密钥 E2E 离线连接分享已完成。多人终端主持端状态机已覆盖活动成员/设备、默认只读、64 KB 帧、严格输出序号、单持有者短租约、输入防重放，以及角色降级后的立即拒绝；内部帧未连接网络。在线账号/邀请/设备目录同步、服务端 RBAC、WSS relay、端到端房间帧加密和真实双设备多人终端仍需正式服务与设备，不声明完成 |
