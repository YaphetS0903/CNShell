...
| 发布 workflow 供应链门禁 | 本机通过，远端修复待复验；CI/release 所有外部 `uses:` 均固定为 40 位 commit SHA，Node `20.20.2`、Rust `1.96.0` 固定；执行 Clippy 的 workflow 显式安装 minimal profile 不包含的组件。15 个 release-script 定向测试验证精确引用、Clippy 组件、拒绝浮动 tag，并保证 `.p12`、`.p8`、私有 release 配置和临时 Keychain 在 Artifact Action 前清理，只有构建与清理均成功才上传。GitHub Actions run `29525490667`（CI #75）证明固定 Action、Node 与 Rust 均成功解析，但 `Frontend and Rust` 因初版未安装 `cargo-clippy` 失败；当前修复须由后续 CI 复验，不把 #75 记为通过 |
