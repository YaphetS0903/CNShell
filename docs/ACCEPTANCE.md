...
| AI、插件、团队协作 | 部分（代码与 loopback 通过，生产/真机待补） | AI、可信插件沙箱、本地团队 RBAC/设备/审计、Keychain 设备密钥 E2E 离线连接分享、relay 服务端、客户端账号/工作区同步和在线多人终端 WebSocket/观看控制 UI 已完成。正式域名/TLS/WSS/邮件/限速/监控/备份恢复和真实双设备跨网络会话仍未完成，不声明生产在线团队服务验收通过 |
...
| 保留验收边界 | 本阶段只验收账号与目录同步；房间 WebSocket 和观看/控制 UI 已在后续增量中接通。正式 TLS 域名、邮件、监控备份和两台真实设备跨网络证据仍未完成 |
...
| 保留验收边界 | 当前只完成同机 loopback 自动化，没有正式 DNS/TLS/WSS、邮件、限速、监控、加密备份/恢复演练，也没有两台真实设备跨网络观看、控制和断网恢复证据；因此仅记为代码与 loopback 通过 |
