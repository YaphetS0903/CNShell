...
| AI、插件、团队协作 | 部分（本机代码完成/在线服务待建） | AI、可信插件沙箱、本地团队 RBAC/设备/审计和 Keychain 设备密钥 E2E 离线连接分享已完成。插件提供逐项显式授权的 `ui`、精确域名 HTTPS、用户 Bookmark 目录、终端选区/待确认输入、脱敏连接元数据和一次性凭据代理 ABI；未知导入、未授权能力、私网解析、重定向、符号链接与越界均阻断。多人终端主持端状态机已覆盖活动成员/设备、默认只读、64 KB 帧、严格输出序号、单持有者短租约、输入防重放，以及角色降级后的立即拒绝；内部帧未连接网络。在线账号/邀请/设备目录同步、服务端 RBAC、WSS relay、端到端房间帧加密和真实双设备多人终端仍需正式服务与设备，不声明完成 |
...
| 自动化门禁 | `npm run check` 通过：前端 46 个文件、127 项测试；Rust 186 项测试；TypeScript、Vite production build、IPC 生成一致性、ESLint 与 `git diff --check` 通过。新增测试覆盖敏感权限默认拒绝和选择性授予、未知导入、燃料耗尽、HTTPS/域名/端口/片段拒绝、相对路径约束及签名插件生命周期 |
