...
| 发布 workflow 供应链门禁 | 通过；CI/release 所有外部 `uses:` 均固定为 40 位 commit SHA，Node `20.20.2`、Rust `1.96.0` 固定；15 个 release-script 定向测试验证精确引用、拒绝浮动 tag，并保证 `.p12`、`.p8`、私有 release 配置和临时 Keychain 在 Artifact Action 前清理，只有构建与清理均成功才上传。GitHub Actions run `29523263973`（CI #74）已验证上一版 updater/回滚门禁；本行新增 workflow 固定仍以本次提交触发的 CI 为准 |
