...
1. 将已通过容器 smoke 的 TLS/WSS 代理模板部署到正式域名和可信证书，并验证公网边界。
2. 代理层登录/注册速率限制配置已经完成；仍需验证真实 SMTP 投递信誉与退信处理、防滥用和
   Alertmanager 通知。邮箱验证协议、哈希存储、会话阻断和客户端入口已经完成，仍需在目标
   邮件供应商上验证投递。
3. 在加密卷、生产 `age` identity、异地存储和隔离恢复主机上执行自动备份与恢复演练，并接入
   日志保留、监控和事故响应。release 供应链脚本、指标、Prometheus/Alertmanager 规则和
   runbook 已完成，CI smoke 不能替代真实值班通知和目标故障域中的恢复证据。
