...
| 发布与文档 | 代码门禁完成，macOS universal 本机重建通过 | macOS universal、Windows x64/ARM64 构建入口与安装资源检查已接入；完整 Apache-2.0 与第三方声明、用户/安全/隐私/排障文档已更新；全量 Rust 主程序 246 项、sidecar 9 项、前端 205 项测试、Clippy 与生产构建通过。当前 macOS App 主程序与 MCP sidecar 均验证为 `x86_64 + arm64` 且 ad-hoc 签名有效 | GitHub Actions 新提交实际运行；Developer ID/Authenticode 正式签名仍属发行环境 |
