# CNshell v0.2.0-beta.4 跨平台 Beta

这是供真实设备测试的未签名预发布版，不是已完成商业代码签名的正式版本。

## 下载选择

- macOS 13 或更高版本、Apple Silicon/Intel：`CNshell_0.2.0-beta.4_universal.dmg`
- Windows 10 22H2（build 19045）或 Windows 11 x64：`CNshell_0.2.0-beta.4_x64-setup.exe`，状态为 **Beta**
- Windows 11 ARM64：`CNshell_0.2.0-beta.4_arm64-setup.exe`，状态为 **Preview**
...
本次 Beta.4 重点验证 Windows 启动不再弹出额外控制台窗口、原生标题为 CNshell、Windows 浅色终端配色正确，以及从 Beta.2 覆盖升级后连接资料与系统凭据保持正确。
