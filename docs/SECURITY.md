...
- 多人终端默认只读，输出/输入帧 64 KB 有界且序号严格单调；控制输入要求 10–300 秒单持有者租约，并在每帧重新校验双方角色、活动设备、epoch、租约和防重放序号。主持端输出与参与端输入都在 Rust 中使用房间 AES-256-GCM 密钥加密并由实际发送设备 Ed25519 签名，relay 只见 envelope 元数据和有界密文。客户端按房间串行生成序号，待发与观看历史各限制 512 帧/4 MiB，重连以服务端游标对账；参与者离开会撤销房间访问和其控制租约。正式公网服务仍需 TLS/WSS、限速、监控、备份恢复和跨设备验收。
