...
## 10. 竞品路线图实施补充（2026-07-12）

- 已完成终端效率包：智能命令栏、会话日志、正则高亮/通知和多主机批量执行。
- 已完成编辑诊断包：CodeMirror 远端编辑、进程/网络诊断、任意嵌套拆分、OpenSSH 配置/密钥和终端安全工具。
- 已完成可独立落地的协议自动化包：同 SSH 会话 SCP 降级、逐连接 Agent 转发、受限任务编排和用户目录加密同步。
- Zmodem、Mosh 与 X11 只在依赖/引擎满足时启用，当前提供真实能力探测和明确安全说明，不降级为不安全的系统命令调用。
- AI、通用插件和团队协作保持延期；不得绕过命令预览、数据最小化、BYOK/本地模型和显式权限前置条件。
- 逐项实现与验收证据以 `docs/COMPETITIVE_ROADMAP.md` 第 7 节和 `docs/ACCEPTANCE.md` 为准。
