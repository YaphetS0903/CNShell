...
pub fn capabilities() -> Vec<ProtocolCapability> {
    ...
    vec![
        ProtocolCapability { id:"zmodem".into(), label:"Zmodem rz/sz".into(), available:true, executable:None, message:"CNshell 内置 Zmodem 协议端；远端安装并运行 rz/sz 后会自动接管控制序列并请求本地文件授权。".into(), security_warning:Some("Zmodem 传输由交互终端发起，文件仍必须通过系统原生选择器授权。".into()) },
        ...
        ProtocolCapability { id:"kermit".into(), label:"Serial Kermit".into(), available:crate::kermit::available(), executable:crate::kermit::helper_path().map(|path|path.to_string_lossy().into_owned()), message:"CNshell 通过内置、受管的 G-Kermit 2.01 sidecar 提供批量二进制传输。".into(), security_warning:Some("接收文件先进入隔离临时目录，验证为有界普通文件后才移入用户选择的目录。".into()) },
        ...
        ProtocolCapability { id:"x11".into(), label:"X11 转发".into(), available:x11.is_ok(), executable:x11.as_ref().ok().cloned(), message:x11.err().unwrap_or_else(||"DISPLAY、xauth 与本地 X Server 均可用；可按可信连接启用真实 X11 channel 转发。".into()), security_warning:Some("远端图形程序可访问本地 X Server；CNshell 使用一次性 cookie 隔离，但仍只应对可信主机开放。".into()) },
        ProtocolCapability { id:"agentForwarding".into(), label:"SSH Agent 转发".into(), available:crate::platform::ssh_agent_available(), executable:std::env::var("SSH_AUTH_SOCK").ok(), message:"可按连接独立启用，连接建立后通过 SSH 协议请求 Agent 转发。".into(), security_warning:Some("远端 root 或被入侵进程可能借用本机 Agent 签名；只对完全可信主机启用。".into()) },
    ]
}
...
pub fn validate_options(
    ...
    if agent_forwarding && !crate::platform::ssh_agent_available() {
        return Err(AppError::Unavailable(
            "当前系统没有可用的 SSH Agent，无法启用 Agent 转发".into(),
        ));
    }
    if mosh_enabled {
