...
pub fn capabilities() -> Vec<ProtocolCapability> {
    ...
    vec![
        ProtocolCapability { id:"zmodem".into(), label:"Zmodem rz/sz".into(), available:true, executable:None, message:"CNshell 内置 Zmodem 协议端；远端安装并运行 rz/sz 后会自动接管控制序列并请求本地文件授权。".into(), security_warning:Some("Zmodem 传输由交互终端发起，文件仍必须通过 macOS 原生选择器授权。".into()) },
