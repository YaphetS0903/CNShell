...
pub(crate) struct SessionInner {
    ...
    x11_state: Option<Box<X11CallbackState>>,
}
...
const LIBSSH2_CALLBACK_X11: c_int = 4;

unsafe extern "C" fn x11_open_callback(
    _session: *mut raw::LIBSSH2_SESSION,
    channel: *mut raw::LIBSSH2_CHANNEL,
    _source_host: *const c_char,
    _source_port: c_int,
    abstract_pointer: *mut *mut c_void,
) {
    if channel.is_null() || abstract_pointer.is_null() || (*abstract_pointer).is_null() {
        return;
    }
    let state = &*((*abstract_pointer).cast::<X11CallbackState>());
    let _ = state.sender.send(channel as usize);
}
...
/// Receiver for server-initiated X11 channels associated with a session.
pub struct X11ChannelReceiver {
    receiver: Receiver<usize>,
    session: Session,
    state_id: usize,
}

impl X11ChannelReceiver {
    /// Wait for an incoming X11 channel until the supplied timeout expires.
    pub fn recv_timeout(&self, timeout: Duration) -> Result<Option<Channel>, Error> {
        match self.receiver.recv_timeout(timeout) {
            Ok(channel) => Channel::from_raw_opt(
                channel as *mut raw::LIBSSH2_CHANNEL,
                None,
                &self.session.inner,
            )
            .map(Some),
...
impl Drop for X11ChannelReceiver {
    fn drop(&mut self) {
        let mut inner = self.session.inner();
        let current = inner
            .x11_state
            .as_deref()
            .map(|state| state as *const X11CallbackState as usize);
        if current != Some(self.state_id) {
            return;
        }
        unsafe {
            let _ = libssh2_session_callback_set2(inner.raw, LIBSSH2_CALLBACK_X11, null_mut());
            *raw::libssh2_session_abstract(inner.raw) = null_mut();
        }
        inner.x11_state = None;
    }
}
...
impl Session {
    ...
    /// Enable delivery of server-initiated X11 channels for this session.
    ///
    /// Only one receiver may be active for a session at a time.
    pub fn x11_channel_receiver(&self) -> Result<X11ChannelReceiver, Error> {
        let (sender, receiver) = mpsc::channel();
        let mut inner = self.inner();
        if inner.x11_state.is_some() {
            return Err(Error::new(
                ErrorCode::Session(raw::LIBSSH2_ERROR_INVAL),
                "an X11 channel receiver is already active",
            ));
        }
        inner.x11_state = Some(Box::new(X11CallbackState { sender }));
        let state_id = inner
            .x11_state
            .as_deref()
            .map(|state| state as *const X11CallbackState as usize)
            .expect("X11 callback state was just inserted");
        unsafe {
            *raw::libssh2_session_abstract(inner.raw) = state_id as *mut c_void;
            let _ = libssh2_session_callback_set2(
                inner.raw,
                LIBSSH2_CALLBACK_X11,
                x11_open_callback as *const () as *mut c_void,
            );
        }
        drop(inner);
        Ok(X11ChannelReceiver {
            receiver,
            session: self.clone(),
            state_id,
        })
