...
impl Channel {
    ...
    /// Request X11 forwarding on this session channel.
    ///
    /// Callers should send a synthetic cookie and validate it before proxying
    /// each incoming X11 channel to a local display.
    pub fn request_x11(
        &mut self,
        ...
        let auth_protocol = auth_protocol.map(CString::new).transpose()?;
        let auth_cookie = auth_cookie.map(CString::new).transpose()?;
        let locked = self.lock();
        locked.sess.rc(unsafe {
            libssh2_channel_x11_req_ex(
                locked.raw,
                single_connection as c_int,
                auth_protocol
                    .as_ref()
